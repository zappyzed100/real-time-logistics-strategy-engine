...
3. 実行前に必須プレースホルダ（`EXPECTED_ACCOUNT`, `RUNNER_RSA_PUBLIC_KEY`, `RUNNER_RSA_PUBLIC_KEY_2`, CIDR）を実値へ置き換えてください。
...
- Terraform 実行ユーザーの公開鍵は `RSA_PUBLIC_KEY` / `RSA_PUBLIC_KEY_2` の 2 スロット運用を前提とします。
