...
-- 1.1 既存 service user がある場合は所有権を DEV_TF_ADMIN_ROLE へ移譲
-- -----------------------------------------------------
EXECUTE IMMEDIATE $$
BEGIN
  SHOW USERS LIKE 'DEV_LOADER_USER';
  IF (EXISTS(SELECT 1 FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())))) THEN
    EXECUTE IMMEDIATE 'GRANT OWNERSHIP ON USER DEV_LOADER_USER TO ROLE DEV_TF_ADMIN_ROLE COPY CURRENT GRANTS';
  END IF;

  SHOW USERS LIKE 'DEV_DBT_USER';
  IF (EXISTS(SELECT 1 FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())))) THEN
    EXECUTE IMMEDIATE 'GRANT OWNERSHIP ON USER DEV_DBT_USER TO ROLE DEV_TF_ADMIN_ROLE COPY CURRENT GRANTS';
  END IF;

  SHOW USERS LIKE 'DEV_STREAMLIT_USER';
  IF (EXISTS(SELECT 1 FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())))) THEN
    EXECUTE IMMEDIATE 'GRANT OWNERSHIP ON USER DEV_STREAMLIT_USER TO ROLE DEV_TF_ADMIN_ROLE COPY CURRENT GRANTS';
  END IF;
END;
$$;
